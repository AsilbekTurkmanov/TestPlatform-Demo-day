...
const defaultResultsMap: Record<string, ResultDetail> = {
  ...
  att_demo_3: {
    ...
    questionReviews: (mockQuestions[mockExams[2].id] || []).map((q) => {
      const correctOpt = q.options.find((o) => o.isCorrect) || q.options[0];
      return {
        questionId: q.id,
        text: q.text,
